...
define([
	...
	var ParallaxbgSwitcherView = Backbone.View.extend({
		...
		onBlockInview: function(event, measurements) {
			var isOnscreen = measurements.percentFromTop < 60 && measurements.percentFromBottom < 60 ;
		...
		showBackground: function() {
			...
			if(Modernizr.csstransitions){
				...
				this.$('.background-switcher-background.active').animate({opacity:0}, 200, function(){ $(this).removeClass('active'); });
				$(this.$backgrounds[this._activeId]).animate({opacity:1}, 200, function(){ $(this).addClass('active'); });
			}
		},
